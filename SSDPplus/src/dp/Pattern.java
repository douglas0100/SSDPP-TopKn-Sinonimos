...
public class Pattern implements Comparable<Pattern>, Serializable {
    ...
    public String[] getAtributos(){
        ...
        // Salvando em um array de strings

        String[] atributos = new String[itens.size()];
        ...
        if (this.similares == null) {
            ...
            if (similar.getQualidade() > this.similares[Pattern.maxSimulares - 1].getQualidade()) {
                if (!this.ehIgualSimilares(similar)) {
                    this.similares[Pattern.maxSimulares - 1] = new Pattern(similar.getItens(),
                            similar.getTipoAvaliacao());
     ...
     * atributo qualidade.
     * Lógica: (1): se this maior que p, (0) se iguais e (-1) se this menos que p
     * (confirmar!)
    ...
    // Imprime regras em texto incluindo similares de cada DP
    public String toString(String[] metricas, boolean imprimirCoberturaDp, boolean imprimirCoberturaDn,
            boolean imprimirSimilares) {
        if (this.itens.size() == 0) {
            return "";
        }
        // Capturando e ordenando conteúdo
        ...
        if (imprimirSimilares && this.similares != null) {
            ...
            for (int i = 0; i < this.similares.length; i++) {
                str.append("   cache(" + i + "):");
                str.append(this.similares[i].toString(metricas, imprimirCoberturaDp, imprimirCoberturaDn,
                        !imprimirSimilares));
                str.append(" ||Similarity:"
                        + df4.format(Avaliador.similaridade(this, this.similares[i], Pattern.medidaSimilaridade))
                        + "||");
                str.append("\n");
            }
        }
